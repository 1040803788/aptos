...
def format_report(context: ForgeContext, result: ForgeResult) -> str:
    ...
    for line in result.output.splitlines():
        if line in ("====json-report-begin===", "====json-report-end==="):
            ...
            if len(error_buffer) == error_length:
                error_buffer.pop(0)
...
def format_comment(context: ForgeContext, result: ForgeResult) -> str:
    ...
    if result.state == ForgeState.PASS:
        forge_comment_header = "### :white_check_mark: Forge test success on `{context.image_tag}`"
    elif result.state == ForgeState.FAIL:
        forge_comment_header = "### :x: Forge test perf regression on `{context.image_tag}`"
    elif result.state == ForgeState.SKIP:
        forge_comment_header = "### :thought_balloon: Forge test preempted on `{context.image_tag}`"
    else:
...
@envoption("BALANCE_CLUSTERS", True)
def test(
    local_p99_latency_ms_threshold: str,
    ...
    if forge_cluster_name is None or balance_clusters:
