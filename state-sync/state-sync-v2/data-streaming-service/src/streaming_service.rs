...
mod streaming_service_tests {
    ...
    async fn test_terminate_data_streams_feedback() {
        // Verify the streams are terminated, even if incorrect feedback is given
        for incorrect_notification_feedback in [false, true] {
            // Create a new streaming service
            let (_, mut streaming_service) =
                tests::streaming_service::create_streaming_client_and_server(false, false, true);
            ...
            for (data_stream_id, data_stream_listener) in &mut stream_ids_and_listeners {
                ...
                loop {
                    ...
                    .await
                    {
                        ...
                        let notification_id = if incorrect_notification_feedback {
                            10101010 // An invalid notification id
                        } else {
                    ...
                    if Instant::now() > timeout_deadline {
                        panic!("Failed to receive a notification from the data stream!");
                    }
                }
            }
